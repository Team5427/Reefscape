...
public class AlignChassisToCenter extends Command {
  ...
  public void execute() {
    ...
    if (DriverStation.isTeleop()) {
      targetPose =
          RobotState.getInstance()
              .getAdaptivePose()
              .nearest(List.of(RobotConfigConstants.kReefCenters));
    }
    ...
    if (driveController.getThetaController().atSetpoint()
        && driveController.getXController().atSetpoint()) speeds = new ChassisSpeeds(0, 0, 0);
    ...
    ChassisSpeeds driverSpeeds =
        swerve.getDriveSpeeds(
            0.0, speeds.vyMetersPerSecond, speeds.omegaRadiansPerSecond, dampener);
