...
public class CascadeIOSim implements CascadeIO {
  ...
  public CascadeIOSim() {
    ...
    cascadeSim =
        new ElevatorSim(
            cascadeMotors.getGearbox(),
            CascadeConstants.kCascadeDriverConfiguration.gearRatio.getMathematicalGearRatio(),
            15.0,
            Units.inchesToMeters(0.5),
            CascadeConstants.kCascadeMinimumHeight.in(Meters),
    ...
    cascadeController =
        new ProfiledPIDController(
            CascadeConstants.kCascadeDriverConfiguration.kP,
            CascadeConstants.kCascadeDriverConfiguration.kI,
            CascadeConstants.kCascadeDriverConfiguration.kD,
            new Constraints(
                CascadeConstants.kCascadeDriverConfiguration.maxVelocity,
                CascadeConstants.kCascadeDriverConfiguration.maxAcceleration));
    cascadeFeedforward =
        new ElevatorFeedforward(
            CascadeConstants.kCascadeDriverConfiguration.kS,
            CascadeConstants.kCascadeDriverConfiguration.kG,
            CascadeConstants.kCascadeDriverConfiguration.kV / (2 * Math.PI),
            CascadeConstants.kCascadeDriverConfiguration.kA / (2 * Math.PI));

    pivotController =
        new ProfiledPIDController(
            CascadeConstants.kPivotConfiguration.kP,
            CascadeConstants.kPivotConfiguration.kI,
            CascadeConstants.kPivotConfiguration.kD,
            new Constraints(
                CascadeConstants.kPivotConfiguration.maxVelocity,
                CascadeConstants.kPivotConfiguration.maxAcceleration));

    pivotFeedforward =
        new ArmFeedforward(
            CascadeConstants.kPivotConfiguration.kS,
            CascadeConstants.kPivotConfiguration.kG,
            CascadeConstants.kPivotConfiguration.kV / (2 * Math.PI),
  ...
  public void setCascadeSetpoint(Distance setpoint) {
    ...
    cascadeMotors.setInputVoltage(
        cascadeController.calculate(cascadeSim.getPositionMeters(), setpoint.in(Meters))
  ...
  public void setPivotSetpoint(Rotation2d setpoint) {
    ...
    pivotMotors.setInputVoltage(
        pivotController.calculate(
                pivotMotors.getAngularPositionRotations(), setpoint.getRotations())
            + pivotFeedforward.calculate(
                pivotMotors.getAngularPositionRad(),
                (pivotMotors.getAngularVelocityRPM() / 60.0) - pivotController.getVelocityError()));
  }
