...
public class EndEffectorIOSim implements EndEffectorIO {
	...
	public EndEffectorIOSim() {
		...
		wristMotor = new DCMotorSim(
				...
				EndEffectorConstants.kWristMotorConfiguration.withFOC
						? DCMotor.getKrakenX60Foc(1)
						: DCMotor.getKrakenX60(1));
		coralRollerMotor = new DCMotorSim(
				LinearSystemId.createDCMotorSystem(
						EndEffectorConstants.kCoralRollerMotorConfiguration.withFOC
								? DCMotor.getKrakenX60Foc(1)
								: DCMotor.getKrakenX60(1),
						coralRollerMotorInertia,
						EndEffectorConstants.kCoralRollerMotorConfiguration.gearRatio.getMathematicalGearRatio()),
				EndEffectorConstants.kCoralRollerMotorConfiguration.withFOC
						? DCMotor.getKrakenX60Foc(1)
						: DCMotor.getKrakenX60(1));
		algaeRollerMotor = new DCMotorSim(
				LinearSystemId.createDCMotorSystem(
						EndEffectorConstants.kAlgaeRollerMotorConfiguration.withFOC
								? DCMotor.getKrakenX60Foc(1)
								: DCMotor.getKrakenX60(1),
						algaeRollerMotorInertia,
						EndEffectorConstants.kAlgaeRollerMotorConfiguration.gearRatio.getMathematicalGearRatio()),
				EndEffectorConstants.kAlgaeRollerMotorConfiguration.withFOC
						? DCMotor.getKrakenX60Foc(1)
						: DCMotor.getKrakenX60(1));
	}
	...
	public void setCoralWristSetpoint(Rotation2d setpoint) {
		EndEffectorConstants.kSIMWristController.setGoal(
	...
	public void setPivotSetpoint(Rotation2d setpoint) {
		EndEffectorConstants.kSIMPivotController.setGoal(
				setpoint.getRotations());
	}

	@Override
	public void updateInputs(EndEffectorIOInputs inputs) {
		pivotMotorVoltage = EndEffectorConstants.kSIMPivotController.calculate(
				pivotMotor.getAngularPositionRotations());
		wristMotorVoltage = EndEffectorConstants.kSIMWristController.calculate(
				wristMotor.getAngularPositionRotations());

		pivotMotor.setInputVoltage(
				MathUtil.clamp(pivotMotorVoltage, -12.0, 12.0));
		wristMotor.setInputVoltage(
				MathUtil.clamp(wristMotorVoltage, -12.0, 12.0));
		...
		inputs.algaeRollerMotorCurrent = Amps.of(
				algaeRollerMotor.getCurrentDrawAmps());
		inputs.algaeRollerMotorLinearVelocity = MetersPerSecond.of(
				(algaeRollerMotor.getAngularVelocityRPM() *
						EndEffectorConstants.kAlgaeRollerMotorConfiguration.finalDiameterMeters *
						Math.PI) /
						60.0);
		inputs.algaeRollerMotorVoltage = Volts.of(
				algaeRollerMotor.getInputVoltage());
		...
		inputs.coralRollerMotorCurrent = Amps.of(
				coralRollerMotor.getCurrentDrawAmps());
		inputs.coralRollerMotorLinearVelocity = MetersPerSecond.of(
				(coralRollerMotor.getAngularVelocityRPM() *
						EndEffectorConstants.kCoralRollerMotorConfiguration.finalDiameterMeters *
						Math.PI) /
						60.0);
		inputs.coralRollerMotorVoltage = Volts.of(
				coralRollerMotor.getInputVoltage());
		...
		inputs.wristAngle = Rotation2d.fromRotations(
				wristMotor.getAngularPosition().in(Rotation));
